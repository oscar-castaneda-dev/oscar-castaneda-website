...
- Background: `--void` or `--graphite`
- Border: `1px solid --carbon`, or none. Radius: 12–16px cards, 8px compact, 4px technical
...
| Secondary   | transparent             | `1px solid --slate`     | `--mist`       | 999px        |
| Ghost       | transparent             | none                    | `--smoke`      | 0            |
...
- Underline preferred (`1px solid --slate` bottom) or full border 8px radius
- Label above: `--label` style (Space Mono, ALL CAPS, `--smoke`)
- Focus: border → `--mist`. Error: border → `--accent`, message below in `--accent`
...
- Dividers: `1px solid --carbon`, full-width. Row padding: 12–16px vertical
- Left: label (Space Mono caps, `--smoke`). Right: value (`--mist`)
- Never alternating row backgrounds. Use dividers.

**Stat rows:** Label left (Space Mono, ALL CAPS, `--smoke`), value right (color = status color), unit adjacent in `--label` size. Trend arrow same color as value.
...
- Header: `--label` style, bottom border `--slate`
...
- Active row: `--graphite` background, left `2px solid --accent` indicator
...
- Labels: Space Mono, ALL CAPS. Active: `--text-display` + dot/underline. Inactive: `--ash`
- Bracket `[ HOME ]  GALLERY  INFO` or pipe `HOME | GALLERY | INFO`
- **Back button:** Circular 40–44px, `--void` bg, thin chevron `<`, top-left 16px from edges

---

## 7. TAGS / CHIPS

- Border: `1px solid --slate`, no fill. Text: Space Mono, `--caption`, ALL CAPS
...
- Container: `1px solid --slate`, pill or 8px rounded
- Active: `--text-display` bg, `--black` text (inverted). Inactive: transparent, `--smoke`
...
- Label: Space Mono/Grotesk, ALL CAPS. Arrows: thin chevrons, `--smoke`, 44px touch
...
- Pill track, circle thumb. Off: `--slate` track, `--ash` thumb
...
**Segments:** Square-ended blocks, no border-radius. Filled = solid status color. Empty = `--carbon` (dark) / `#E0E0E0` (light).
...
- **Bar charts:** Vertical, white fill, `--carbon` remainder. Square ends.
...
**Charts:** Line 1.5–2px `--text-display`, average dashed 1px `--smoke`. Axis labels: Space Mono, `--caption`. Grid: `--carbon`, horizontal only. No area fill, no legend boxes — label lines directly.

---

## 13. WIDGETS (DASHBOARD CARDS)

- `--void` bg, 16px radius. Hero metric: large Doto/Space Mono, left-aligned
...
- **Modals:** Backdrop `rgba(0,0,0,0.8)`, dialog `--void` + `1px solid --slate` + 16px radius, centered max 480px. Close: `[ X ]` top-right ghost button.
- **Bottom sheets:** `--void`, 2px handle bar centered, 16px top radius, drag-to-dismiss. Full-page sheets: title centered + dismiss button right, sections with `--smoke` headings.
- **Dropdowns:** `--graphite`, `1px solid --slate` 8px radius, 44px items. Selected: left 2px accent bar. No shadow.
...
- **Empty:** Centered, 96px+ padding. Headline `--smoke`, 1 sentence description `--ash`. Optional dot-matrix illustration. No mascots.
- **Loading:** Segmented spinner (hardware-style), or segmented bar + percentage. No skeletons — use `[LOADING]` bracket text.
- **Disabled:** Opacity 0.4 or `--ash`. Borders fade to `--carbon`.
